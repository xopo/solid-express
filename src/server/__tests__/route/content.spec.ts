...
describe("/api/content", () => {
    ...
    describe("given a user is logged in", () => {
        ...
        beforeAll(async () => {
            ...
            vi.spyOn(dbQuery, "dbGetUserByName").mockImplementationOnce(
                (_name: string) =>
                    Promise.resolve({ name: "daniel", id: 1, pass: "" }),
            );
            loginResponse = await request
                .post("/api/login")
                .send({ name: "daniel", pass: btoa("23423") });
        });
        ...
        describe("given the user post a correct url", () => {
            it("will add the media to db", async () => {
                const result = await request
                    ...
                    .send({
                        url: "https://www.youtube.com/watch?v=egkG6ynXs-0",
                    })
                    .set("cookie", loginResponse.header["set-cookie"]);
                expect(result.statusCode).toBe(200);
            });
