...
export function selectThumbnail(thumbs: ThumbType[], defaultThumbnail: string) {
    // get mq
    const mqThumbs = thumbs.filter(ob => ob.url.includes('mq') && ob.url.includes('webp'));
    if (mqThumbs.length) return mqThumbs[0]?.url;

    // or get hq
    const hqThumbs = thumbs.filter(ob => ob.url.includes('hq') && ob.url.includes('webp'));
...
export async function renameFile(media_id: string) {
    ...
    if (target) {
        ...
        if (files?.length) {
            const downloads = files.filter(f => f.isFile() && f.name.includes(media_id));
            if (downloads?.length) {
