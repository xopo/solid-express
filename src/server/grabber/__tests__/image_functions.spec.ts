import { getExtension, newImagePath } from "../grab";
    ...
    describe("Get remote file extension", () => {
        it("Extracts correctly the extension from img url", () => {
            const listOfUrls: Record<string, string> = {
                "https://i.ytimg.com/vi/i7wveOu5hkQ/hqdefault.jpg?sqp=-oaymwEWCKgBEF5IWvKriqkDCQgBFQAAiEIYAQ==&rs=AOn4CLA0mNDkZLqTBqxyvkrWeUSUI8v_FA":
                    "jpg",
                ...
                "https://ak2.rmbl.ws/s8/1/h/C/8/m/hC8mq.aiEB-small-Far-right-homeschool-mom-wi.jpg":
                    "jpg",
    ...
    describe("Prepare path for files with newPathFileName", () => {
        it("Returns correct file when all parameters are set", () => {
            ...
            for (let test of listOfParams) {
                expect(newImagePath(test.title)).toBe(location + test.expect);
            }
        });
    });
});
