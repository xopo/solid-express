...
export const uriSchema = object({
    body: object({
        url: string("Url required", [minLength(5), ValidateUrl]),
    }),
});

export const tagsSchema = object({
    body: object({
        label: string("label required", [minLength(3)]),
    }),
});

export const tagsToggleSchema = object({
    body: object({
...
export const postDateUser = object({
    query: object({
        date: CustomDate,
        table: string("Lipseste invitatul", [
            toTrimmed(),
            minLength(4),
            maxLength(10),
        ]),
