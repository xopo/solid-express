...
export default function Login() {
    ...
    const checkUserName = async () => {
        ...
        const response = (await apiCheckUserUnique(nameVal)) as {
            error?: string;
        };
        if (response.error) {
    ...
    const [_checkDupe, { refetch: refetchDupe }] =
        createResource(checkUserName);
        ...
        <div class="login" classList={{ register: !isLogin() }}>
            <form>
                ...
                <label>
                    ...
                    />
                    {errors?.parola && (
                        <span class="error">{errors.parola}</span>
                    )}
                </label>
                <Show when={!isLogin()}>
                    <label class="conf">
                ...
                <button
                    ...
                    disabled={
                        !!errors.nume.length ||
                        !!errors.parola.length ||
                        !!confErr().length
                    }
                    type="button"
                >
                    {isLogin() ? "Login" : "Inregistreaza"}
                </button>
                <Show
                    when={isLogin()}
                    fallback={<a href={`${BASE_URL}login`}>inapoi la login</a>}
                >
